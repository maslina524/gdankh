...
if __name__ == "__main__":
    ...
    data = json.loads(open("flat_schema.json", "r").read()).get("classes")

    ret = {}
    for cls in data:
        fields = data.get(cls).get("fields")
        for field in fields:
            if not str(field.get("id")) in ret:
                ret[str(field.get("id"))] = [field.get("type").lower(), field.get("name")]

    ret = dict(sorted(ret.items(), key=lambda x: int(x[0])))
    
    parts = []
    for id in ret:
       parts.append(f"    \"{id}\": \"{ret.get(id)[0]}\",   # {ret.get(id)[1]}")
